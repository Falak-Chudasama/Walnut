...
app.post('/ai/groq', async (req: Request, res: Response) => {
    try {
        const { prompt, model, context, needReasoning, temperature, top_p } = req.body;
        const requestedModel: string = groqModels[model];
    
        if (!prompt || !context || !requestedModel) {
            return res.status(400).send('Prompt, context, and a valid model are required.');
        }

        // --- KEY FIX ---
        // Pass the temperature and top_p from the request to the API function.
        const result: object = await groqAPI(prompt, requestedModel, context, needReasoning, temperature, top_p);
...
app.post('/ai/summarize', async (req: Request, res: Response) => {
    try {
        // --- KEY FIX ---
        // Also allow the summarizer to receive temperature settings for reliable JSON.
        const { context, model, temperature, top_p } = req.body;
        const requestedModel: string = groqModels[model];
    
        if (!context || !requestedModel) {
            return res.status(400).send('Context and a valid model are required.');
        }

        // Assuming your summarizer API is also updated to accept these params.
        const result: string = await groqAPISummarizer(context, requestedModel, temperature, top_p);
